...
| Variable | Value |
|----------|--------|
| `VITE_API_BASE_URL` | Leave **empty** for `skyport-home.vercel.app` (app uses same-origin `/api/*` to Core). Set only for custom domains or preview Web → preview Core (see [VERCEL_URLS.md](./VERCEL_URLS.md)). |

Auth always goes through Skyport-Core; there is no separate `VITE_USE_BACKEND_AUTH` flag.
